...
A prompt can be a text string (such as a question that you ask an LLM or a command), and can include results from a search. <br>Here, you can use the `UTL_TO_GENERATE_TEXT` function from either the `DBMS_VECTOR` or the `DBMS_VECTOR_CHAIN` package, depending on your use case. 
...
2. Call `UTL_TO_GENERATE_TEXT`: <br>Here, the cohere.command-r-16k model is used. You can replace `model` with your own value, as required. 
...
1. Call `CREATE_CREDENTIAL` to create and store a credential. <br>Cohere, Google AI, Hugging Face, OpenAI, and Vertex AI require the following authentication parameter: <br>`{ "access_token": "*``*" }`<br>You will later refer to this credential name when declaring JSON parameters for the `UTL_TO_GENERATE_TEXT` call. 
```
