...
2. Call `DBMS_VECTOR.UTL_TO_EMBEDDING`: <br>Here, the cohere.embed-english-v3.0 model is used. You can replace `model` with your own value, as required. 
...
1. Run `DBMS_VECTOR.CREATE_CREDENTIAL` to create and store a credential. <br>Cohere, Google AI, Hugging Face, OpenAI, and Vertex AI require the following authentication parameter: <br>`{ "access_token": "*``*" }`<br>You will later refer to this credential name when declaring JSON parameters for the `UTL_to_EMBEDDING` call. 
```
