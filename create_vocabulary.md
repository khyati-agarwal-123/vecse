...
* **WordPiece**: <br>Vocabulary file (`vocab.txt`) for the "bert-base-uncased" (English) or "bert-base-multilingual-cased" model 

* **Byte-Pair Encoding (BPE)**: <br>Vocabulary file (`vocab.json`) for the "GPT2" model 
...
* **SentencePiece**: <br>Vocabulary file (`tokenizer.json`) for the "xlm-roberta-base" model 
...
* You can create a vocabulary based on the tokens loaded in the `schema.table.column`, using a user-specified vocabulary name (`vocabulary_name`). <br>After loading your vocabulary data, you can use the `by vocabulary` chunking mode (with `VECTOR_CHUNKS` or `UTL_TO_CHUNKS`) to split input data by counting the number of tokens. 
