...
Vertex AI requires the following authentication parameter: <br>`{ "access_token": "*``*" }`
```
begin
...
You will later compare your query image to this database of images. <br>Here, you first create a `docs` table with `id` and `content` columns. Then, using the `to_blob` function, you convert the contents of your image files into `BLOB` for storing into `content` (blob column), reading the files from the `VEC_DUMP` directory. 
```
drop table docs;
```
```
...
First, upload your query image to the `VEC_DUMP` directory. <br>Then, use `UTL_TO_EMBEDDING` to vectorize that image. Here, the input is specified as `parrots.jpg` (with a pointer to the `VEC_DUMP` directory), the modality is specified as `image`, and the provider-specific embedding parameters for Vertex AI are passed as JSON. 
