...
To find a bird that is similar to a given image or text input, using similarity search: 
  ...
  1. Connect as a local user and prepare your data dump directory. 
    ...
    5. Set the HTTP proxy server, if configured. 
        
                ```
        EXEC UTL_HTTP.SET_PROXY(':');
        ```

    6. Grant connect privilege to ` docuser ` for allowing connection to the host, using the ` DBMS_NETWORK_ACL_ADMIN ` procedure. 
...
This example uses ` * ` to allow any host. However, you can explicitly specify the host that you want to connect to. 
    ...
    7. Set up credentials for Vertex AI. 
...
Here, replace *``*  with your own value: 
    ...
    8. Create a relational table ( ` docs ` ) and insert some images in it. 
...
Here, you first create a ` docs ` table with ` id ` and ` content ` columns. Then, using the ` to_blob ` function, you convert the contents of your image files into ` BLOB ` for storing into ` content ` (blob column), reading the files from the ` VEC_DUMP ` directory. 
    ...
    9. Get image embedding for your query image ( ` parrots.jpg ` ): 
...
> **note:** For a list of all supported REST endpoints, see [ Supported Third-Party Provider Operations and Endpoints ](supported-third-party-provider-operations-and-endpoints.html#GUID-BE3EE403-CD10-4708-A15F-EFB1FA69DF09) . 
    ...
    10. Search using an image input. Here, input is the generated embedding for your query image ( ` parrots.jpg ` ): 
...
This query shows that ` ID 2 ` ( ` eagle.jpg ` ) is the closest match to the given image input, while ` ID 1 ` ( ` cat.jpg ` ) is less similar. 
    ...
    11. Search using a text input. Here, input is the image's description as " ` bald eagle ` ": 
...
**Related Topics**
       ...
       * [ UTL_TO_EMBEDDING and UTL_TO_EMBEDDINGS ](utl_to_embedding-and-utl_to_embeddings-dbms_vector_chain.html#GUID-C6439E94-4E86-4ECD-954E-4B73D53579DE)
