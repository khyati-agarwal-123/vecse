...
![Description of bird.jpg follows](https://docs.oracle.com/en/database/oracle/oracle-database/23/vecse/img/bird.jpg)[ Description of the illustration bird.jpg ](https://docs.oracle.com/en/database/oracle/oracle-database/23/vecse/img_text/bird.md)
  ...
  1. Connect to Oracle Database as a local user. 
    ...
    5. Grant connect privilege to ` docuser ` for allowing connection to the host, using the ` DBMS_NETWORK_ACL_ADMIN ` procedure. 
...
This example uses ` * ` to allow any host. However, you can explicitly specify the host that you want to connect to. 
    ...
    6. Create a local directory ( ` DEMO_DIR ` ) to store your image file: 
...
Upload the image file (for example, ` bird.jpg ` ) to the directory. 
    ...
    7. Set up your credentials for the REST provider that you want to access and then call ` UTL_TO_GENERATE_TEXT ` : 
...
> **note:** For a list of all supported REST endpoint URLs, see [ Supported Third-Party Provider Operations and Endpoints ](supported-third-party-provider-operations-and-endpoints.html#GUID-BE3EE403-CD10-4708-A15F-EFB1FA69DF09) . 
