...
1. Call `CREATE_CREDENTIAL` to create and store a credential. <br>Google AI, Hugging Face, OpenAI, and Vertex AI require the following authentication parameter: <br>`{ "access_token": "*``*" }`<br>You will later refer to this credential name when declaring JSON parameters for the `UTL_TO_GENERATE_TEXT` call. 
```
